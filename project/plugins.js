...
if (core.hasFlag('jcpp')) { core.setFlag('zjyzy',-Math.floor(core.getFlag('zjczy', 1))); }
	...
	setInterval(function () {
		...
		if (core.status.hero.exp <= 0) {
			...
			core.status.hero.exp += Math.floor(Math.pow(10, Math.pow(core.status.hero.hp, core.getFlag('lvzs', 0.45))) * Math.max(core.status.hero.hp - ss1, 1)) / 5;
		}
...
if (core.getFlag('zdsju5', 0) === 1) {
	...
	setInterval(function () {
		...
		//基础
		core.setFlag('lvpzy', Math.floor(Math.max(Math.pow(Math.max(core.status.hero.hp - 29, 0), core.getFlag('pexp', 1)))), 0);
		core.setFlag('lvsjzy', Math.max(Math.max((core.status.hero.hp - 97), 0) / 3), 0);
		core.setFlag('pzsjzy', Math.max(Math.max(Math.log10(Math.max(core.status.hero.def / 5000, 1)) / Math.log10(2), 0)), 0);
		//同步
		core.setFlag('zczy', Math.floor(Math.pow(core.getFlag('gczy', 1) * Math.max(core.getFlag('sjtz2', 1) + 1, 1) * core.itemCount("I848") >= 1 ? core.itemCount("I848") * 4 + 1 : 1 * Math.max(core.getFlag('pczy', 1), 1) * Math.pow(core.getFlag('gcfw', 1), 2) * Math.max(core.getFlag('bjczy', 1), 1) * Math.max(core.getFlag('sjczy', 1), 1), 1 - core.getFlag('jrsjtz2', 0) / 5)));
		core.setFlag('zjyzy', Math.floor(Math.pow(core.getFlag('gjyzy', 1) * Math.max(core.getFlag('sjtz1', 1) + 1, 1) * core.itemCount("I848") >= 2 ? core.itemCount("I848") * 4 + 1 : 1 * Math.max(core.getFlag('pjyzy', 1), 1) * Math.max(core.getFlag('bjjyzy', 1), 1) * Math.max(core.getFlag('sjjyzy', 1), 1) * Math.max(core.getFlag('jceff', 1), 1), 1 - core.getFlag('jrsjtz2', 0) / 5)));
		core.setFlag('zpzy', Math.floor(Math.max(Math.max(core.getFlag('lvpzy', 1), 1) * Math.max(core.getFlag('ppzy', 1), 1) * Math.max(core.getFlag('bjpzy', 1), 1) * Math.max(core.getFlag('sjpzy', 1), 1) * (1 - core.getFlag('jrsjtz3', 0)))));
		core.setFlag('zbjzy', Math.max(core.itemCount("I848") >= 4 ? core.itemCount("I848") - 2 : 1 * core.getFlag('pbjzy', 1), 1) * core.itemCount("I848") >= 6 ? 2 : 1 * Math.max(core.getFlag('sjbjzy', 1), 1));
		core.setFlag('zsjzy', Math.floor(Math.pow(Math.max(core.getFlag('lvsjzy', 1), 0) * Math.max(core.getFlag('pzsjzy', 1), 0), Math.max(core.getFlag('sjexp', 0.5), 0)) * Math.max(core.getFlag('sjsjzy', 1), 1) * Math.max(core.getFlag('bjsjzy', 1), 1)));
		core.setFlag('zjczy', Math.floor(core.getFlag('gjczy', 1) * Math.max(core.getFlag('pjczy', 1), 1) * Math.max(core.getFlag('sjjczy', 1), 1) * core.itemCount("I848") >= 3 ? core.itemCount("I848") * 4 + 1 : 1 * Math.max(core.getFlag('bjjczy', 1), 1)));
		core.updateStatusBar(true);
	}, 50);
}
}